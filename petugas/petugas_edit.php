...
$db = new Database();
$id = $_GET['id'];
$row = $db->conn->query("SELECT * FROM petugas WHERE id_user=".$id)->fetch_assoc();
...
if(isset($_POST['update'])){
    ...
    $level = $_POST['level'];
    $stmt = $db->conn->prepare("UPDATE petugas SET nama_user=?, username=?, password=?, level=? WHERE id_user=?");
...
<div class="container">
    ...
    <form method="POST">
        ...
        <input type="text" id="nama" name="nama" value="<?= htmlspecialchars($row['nama_user']) ?>" required />
